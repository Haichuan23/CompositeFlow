...
class PureFlowPolicy(object):
    ...
    def update_q_functions(self, state_batch, action_batch, reward_batch, nextstate_batch, not_done_batch, writer=None):
        ...
        loss = self.weight * F.mse_loss(q_1, value_target, reduction='none') + self.weight * F.mse_loss(q_2, value_target, reduction='none')
        loss = loss.mean()
        return loss
    ...
    def update_policy_and_temp(self, state_batch, src_state_batch, src_action_batch):
        ...
        # p_w * (self.alpha * logprobs_batch - qval_batch).mean() + F.mse_loss(pred_src_act, src_action_batch)
    ...
    def pretrain_source_flow(self, src_replay_buffer, batch_size):
        ...
        self.dynamics_model.train_source_flow_matching(src_replay_buffer, holdout_ratio=self.config['flow_matching_holdout_ratio'], n_epochs=self.config['flow_matching_training_max_epochs'], batch_size=self.config['flow_matching_batch_size'], lr=self.config['flow_matching_lr'])
    ...
    def train_adaptation_flow(self, src_replay_buffer, tar_replay_buffer, batch_size):
        ...
        self.dynamics_model.train_adaptation_flow_matching(tar_replay_buffer, holdout_ratio=self.config['flow_matching_holdout_ratio'], n_epochs=self.config['flow_matching_training_max_epochs'], batch_size=self.config['flow_matching_batch_size'], lr=self.config['flow_matching_lr'])
    ...
    def train(self, src_replay_buffer, tar_replay_buffer, batch_size=128, writer=None):
        # print(f"Train gets called, total iteration is {self.total_it}")
        if self.total_it == 0:
        ...
        if self.upsample_src:
            src_state, src_action, src_next_state, src_reward, src_not_done = src_replay_buffer.sample(int(batch_size/(1-self.config['filter_percent'])))
        else:
        ...
        if self.total_it > self.start_gate_src_sample:
            ## output: [batch_size, 1]
            dynamics_gap = self.dynamics_model.estimate_dynamics_gap_sample_level(src_state, src_action, src_next_state)
            threshold = torch.quantile(dynamics_gap, self.config['filter_percent'])
            mask = dynamics_gap < threshold
            normalized_gap = (dynamics_gap - dynamics_gap.max()) / (dynamics_gap.max() - dynamics_gap.min() + 1e-8)  # ∈ [-1, 0]
          
            self.weight = torch.exp(self.config['alpha'] * normalized_gap)
