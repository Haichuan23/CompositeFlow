...
class FlowPolicy(object):
    ...
    def train(self, src_replay_buffer, tar_replay_buffer, batch_size=128, writer=None):
        ...
        if self.total_it > self.start_gate_src_sample:
            ## output: [batch_size, 1]
            dynamics_gap = self.dynamics_model.estimate_dynamics_gap_sample_level(src_state, src_action, src_next_state) # [batch_size]
            threshold = torch.quantile(dynamics_gap, self.config['filter_percent']) # [1]
            mask = dynamics_gap < threshold # [batch_size]
