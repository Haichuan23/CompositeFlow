    ...
    if 'hopper' in env_name:
        if env_name == 'hopper':
            ...
            if extreme:
                suffix += "_extreme"
            return TimeLimit(
                HopperEnv(xml_file=f"{str(Path(__file__).parent.absolute())}/assets/{env_name}{suffix}.xml",),
        ...
        if env_name == 'halfcheetah':
            ...
            if extreme:
                suffix += "_extreme"
            return TimeLimit(
                HalfCheetahEnv(xml_file=f"{str(Path(__file__).parent.absolute())}/assets/{env_name}{suffix}.xml",),
                max_episode_steps=1000            
            )
        elif 'noise' in env_name:
            # todo: the modification is directly applied on the executed action, no need to modify the xml file itself
        ...
        if env_name == 'walker2d':
            ...
            if extreme:
                suffix += "_extreme"
            return TimeLimit(
                Walker2dEnv(xml_file=f"{str(Path(__file__).parent.absolute())}/assets/{env_name}{suffix}.xml",),
                max_episode_steps=1000            
            )
        elif 'noise' in env_name:
            # todo: the modification is directly applied on the executed action, no need to modify the xml file itself
